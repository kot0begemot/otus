...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        X_dot_weights = X.dot(self.w)
        preds = self.sigmoid(X_dot_weights)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        # Cost function https://ru.wikipedia.org/wiki/Логистическая_регрессия#Подбор_параметров
        ...
        size_train = X_batch.shape[0]
        loss, dw = loss / size_train, dw / size_train

        # Add regularization to the loss and gradient.
        # Note that you have to exclude bias term in regularization.

        loss += (np.sum(self.w[:-1] ** 2) / 2) * reg
        dw[:-1] += self.w[:-1] * reg
