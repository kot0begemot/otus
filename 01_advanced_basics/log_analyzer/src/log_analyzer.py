...
def get_last_logfile(log_dir=config['LOG_DIR']):
    if not log_dir.exists() or not log_dir.is_dir():
        raise FileNotFoundError("Wrong log dir path")

    logfile = None
    log_files = {re.findall('nginx-access-ui.log-([^\.]+)', f)[0]: f for f in os.listdir('.'+log_dir) if re.match(r'nginx-access-ui.log-', f)}
    logfile = log_files[max(log_files, key=int)]

    return logfile

def setup_logging(filename=log_path):
